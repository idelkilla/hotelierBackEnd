...
export async function getProfile(req, res, next) {
  try {
    ...
    const { rows: personaRows } = await db.query(
      `SELECT
         p."ID_PERSONA", p."NOMBRE_COMPLETO", p."APELLIDOS", p."SEGUNDO_NOMBRE",
         p."NUM_VIAJERO_CONOCIDO", p."NUM_DHS_TRIP",
         p."CONTACTO_EMERGENCIA_NOMBRE", p."CONTACTO_EMERGENCIA_TEL",
         p."ID_UBICACION",
         u."NOMBRE"  AS "UBICACION_NOMBRE",
         ci."ID_CIUDAD", ci."NOMBRE" AS "CIUDAD_NOMBRE",
         pa."ID_PAIS", pa."NOMBRE"  AS "PAIS_NOMBRE"
    ...
    const { rows: telRows } = await db.query(
      `SELECT "NUMERO_TELEFONICO" FROM public."TELEFONO"
       WHERE "ID_PERSONA" = $1 AND "ESTADO_TELEFONO" = 'A' LIMIT 1`,
      [idPersona]
    )
    ...
    const { rows: bioRows } = await db.query(
      `SELECT db."SANGRE", db."ESTATURA", db."PESO",
    ...
    return res.json({
      ...
      email,
      telefono_numero:              telRows[0]?.NUMERO_TELEFONICO      ?? '',
...
export async function updateProfile(req, res, next) {
  ...
  try {
    ...
    console.log('updateProfile payload:', JSON.stringify(req.body, null, 2))
    ...
    const {
      ...
      id_ubicacion, email, telefono_numero,
    ...
    // 1. PERSONA
    const hasPersona = [nombre_completo, apellidos, segundo_nombre,
      num_viajero_conocido, num_dhs_trip,
      contacto_emergencia_nombre, contacto_emergencia_tel]
      .some(v => v !== undefined) || id_ubicacion !== undefined
    ...
    if (hasPersona) {
      ...
      await client.query(
        ...
        [
          nombre_completo ?? null, apellidos ?? null, segundo_nombre ?? null,
          num_viajero_conocido ?? null, num_dhs_trip ?? null,
          contacto_emergencia_nombre ?? null, contacto_emergencia_tel ?? null,
          id_ubicacion ?? null, idPersona
        ]
      )
    }
    ...
    if (email !== undefined) {
      await client.query(
        `UPDATE public."USUARIO" SET "CORREO_ELECTRONICO" = COALESCE($1, "CORREO_ELECTRONICO")
         WHERE "ID_USUARIO" = $2`,
    ...
    if (telefono_numero !== undefined) {
      ...
      if (telRows.length) {
        await client.query(
          `UPDATE public."TELEFONO" SET "NUMERO_TELEFONICO" = $1 WHERE "ID_TELEFONO" = $2`,
    ...
    // 4. CLIENTE
    if ([genero, fecha_nacimiento, descripcion_personal].some(v => v !== undefined)) {
      const { rows: cliRows } = await client.query(
        `SELECT 1 FROM public."CLIENTE" WHERE "ID_CLIENTE" = $1`, [idPersona]
      )
      if (cliRows.length) {
        await client.query(
          `UPDATE public."CLIENTE" SET
           ...
           WHERE "ID_CLIENTE" = $4`,
          [genero ?? null, fecha_nacimiento ?? null, descripcion_personal ?? null, idPersona]
        )
      }
    }

    // 5. DATOS_BIOGRAFICOS
    if ([sangre, estatura, peso].some(v => v !== undefined)) {
      const { rows: bioRows } = await client.query(
        `SELECT 1 FROM public."DATOS_BIOGRAFICOS" WHERE "ID_PERSONA" = $1`, [idPersona]
      )
      if (bioRows.length) {
    await client.query(
          `UPDATE public."DATOS_BIOGRAFICOS" SET
             "SANGRE"   = COALESCE($1::varchar(3), "SANGRE"),
             "ESTATURA" = COALESCE($2::char(5),    "ESTATURA"),
             "PESO"     = COALESCE($3::char(5),    "PESO")
           WHERE "ID_PERSONA" = $4`,
    ...
    // 6. DOCUMENTACION
    if ([numero_documento, fecha_emision, fecha_expiracion, emisor].some(v => v !== undefined)) {
      const { rows: docRows } = await client.query(
        `SELECT 1 FROM public."DOCUMENTACION" WHERE "ID_PERSONA" = $1`, [idPersona]
      )
      if (docRows.length) {
        await client.query(
          `UPDATE public."DOCUMENTACION" SET
           ...
           WHERE "ID_PERSONA" = $5`,
          [numero_documento ?? null, fecha_emision ?? null, fecha_expiracion ?? null, emisor ?? null, idPersona]
        )
      } else {
        await client.query(
          ...
          [idPersona, numero_documento ?? null, fecha_emision ?? null, fecha_expiracion ?? null, emisor ?? null]
        )
      }
    }
